...
def main():
    ...
    Class = st.text_input('Percentage of class attendance', placeholder="e.g., 40")
    ...
    if st.button('Predict'):
        ...
        if Class:
            try:
                ...
                st.error("Please enter a valid number for Percentage of class attendance.")
                error = True
        else:
            st.error("Percentage of class attendance is required.")
            error = True
        ...
        if not error:
            ...
            input_data = pd.DataFrame([[Age, Library, Class, Extracurricular]], 
                                      columns=['Age', 'Library_weekly_hours', 'Class_weekly_attendance_percentage', 'Extra-curricular_weekly_hours'])
